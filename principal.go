...
type Cidade struct {
	Cidade  string    `json:"cidade,omitempty"`
	UF      string    `json:"uf,omitempty"`
	Cliente []Cliente `json:"clientes,omitempty"`
}
...
func BuscarCidade(w http.ResponseWriter, r *http.Request) {
	cidadeURL := r.URL.Query().Get("cidade")
	db := conectaNoBancoDeDados()
	selecaoPorCidade, err := db.Query("select p.id, p.nome, e.cidade, e.uf from tb_endereco e left join tb_pessoa p on e.id=p.endereco_id where e.cidade=$1", cidadeURL)
	mensagemErro(err)
	cidadec := Cidade{}
	for selecaoPorCidade.Next() {
		var (
			id               int
			nome, cidade, uf string
		)
		err = selecaoPorCidade.Scan(&id, &nome, &cidade, &uf)
		mensagemErro(err)
		cidadec.Cidade = cidade
		cidadec.UF = uf
		cliente := Cliente{}
		cliente.Id = id
		cliente.Nome = nome
		cidadec.Cliente = append(cidadec.Cliente, cliente)
	}
	defer db.Close()
	//fmt.Println(cidadec)
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(cidadec)
}
...
func main() {
	...
	rotas.HandleFunc("/pessoas/cidade/", BuscarCidade).Methods("GET")
